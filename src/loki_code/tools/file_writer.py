...
from .types import (
    ...
    SecurityLevel, ConfirmationLevel, InputValidationSchema, OutputValidationSchema
)
...
class FileWriterTool(SimpleFileTool):
    ...
    def get_schema(self) -> ToolSchema:
        ...
        return ToolSchema(
            ...
            confirmation_level=ConfirmationLevel.PROMPT,
            input_schema=InputValidationSchema.create_schema(
                properties={
                    ...
                    "create_backup": InputValidationSchema.boolean_field("Create backup before overwriting", True),
                    "auto_format": InputValidationSchema.boolean_field("Auto-format code files", True),
                    "validate_syntax": InputValidationSchema.boolean_field("Validate syntax before writing", True),
                    "max_size_mb": InputValidationSchema.integer_field("Maximum file size in MB", minimum=1, maximum=1000),
                    "insert_line": InputValidationSchema.integer_field("Line number for insert mode (1-based)", minimum=1),
                    "create_dirs": InputValidationSchema.boolean_field("Create parent directories if needed", True)
                },
    ...
    async def execute(self, inputs: Dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        try:
            ...
            processed_content, formatting_applied, syntax_valid, warnings = await self._process_content(
                writer_input.content, file_path, writer_input
            )
    ...
    def _parse_inputs(self, inputs: Dict[str, Any]) -> FileWriterInput:
        ...
        try:
            ...
            raise ToolValidationError(f"Missing required parameter: {e}")
        except Exception as e:
            raise ToolValidationError(f"Invalid input parameters: {e}")
    ...
    async def _validate_security(self, writer_input: FileWriterInput, context: ToolContext) -> None:
        ...
        # Path validation
        if not self._is_safe_path(file_path, context.workspace_path):
            raise ToolSecurityError(f"File path is outside allowed workspace: {file_path}")
        ...
        if content_size_mb > writer_input.max_size_mb:
            raise ToolSecurityError(f"Content size ({content_size_mb:.1f}MB) exceeds limit ({writer_input.max_size_mb}MB)")
        
        # Check for potentially dangerous content
        if self._contains_dangerous_content(writer_input.content):
            raise ToolSecurityError("Content contains potentially dangerous patterns")
        
        # Mode-specific validation
        if writer_input.mode == "insert" and writer_input.insert_line is None:
            raise ToolValidationError("insert_line is required for insert mode")
    ...
    async def _create_backup(self, file_path: Path) -> BackupInfo:
        ...
        try:
            ...
            raise ToolExecutionError(f"Backup creation failed: {e}")
    
    async def _process_content(self, content: str, file_path: Path, writer_input: FileWriterInput) -> Tuple[str, bool, bool, List[str]]:
        ...
        try:
            ...
            if writer_input.validate_syntax and language:
                try:
                    ...
                    tree = self.parser.parse_string(processed_content, language)
    ...
    async def _write_file(self, file_path: Path, content: str, writer_input: FileWriterInput) -> Tuple[str, int, int]:
        ...
        try:
            ...
            self.logger.info(f"File {operation}: {file_path} ({bytes_written} bytes, {lines_written} lines)")
    ...
    async def _generate_suggestions(self, file_path: Path, writer_input: FileWriterInput, context: ToolContext) -> List[str]:
        ...
        try:
            ...
            if file_path.suffix == '.py':
                suggestions.append("Consider running 'python -m py_compile' to check for syntax errors")
